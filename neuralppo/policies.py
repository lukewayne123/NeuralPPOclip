...
class ActorCriticPolicy2(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False, temperature: float = 1.0) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        #distribution = self.action_dist.proba_distribution(action_logits=energy_function/temperature)
        distribution, energy_function = self._get_action_dist_from_latent(latent_pi, latent_sde=latent_sde, temperature=temperature)
        actions = distribution.get_actions(deterministic=deterministic)

        log_prob = distribution.log_prob(actions)
        return actions, values, log_prob, energy_function.squeeze(-1)
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, latent_sde: Optional[th.Tensor] = None, temperature: float = 1.0) -> Distribution:
        """
        ...
        """
        energy_function = self.action_net(latent_pi)
        mean_actions = energy_function / temperature
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            # Only use this
            return self.action_dist.proba_distribution(action_logits=mean_actions), energy_function
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor, temperature: float) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        """
        latent_pi, latent_vf, latent_sde = self._get_latent(obs)
        distribution, energy_function = self._get_action_dist_from_latent(latent_pi, latent_sde, temperature)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
        return values, log_prob, distribution.entropy(), energy_function.squeeze(-1)
